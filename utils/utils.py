...
def normalize_vector( v, use_gpu=True):
    batch=v.shape[0]
    v_mag = tf.sqrt(v.pow(2).sum(1))# batch
    v_mag = tf.maximum(v_mag, tf.Variable(tf.Tensor([1e-8])))
    
    v_mag = tf.reshape(v_mag,(batch,1).expand(batch,v.shape[1]))
    v = v/v_mag
    return v
...
def cross_product( u, v):
    ...
    out = tf.concat([tf.reshape(i,batch,1), tf.reshape(j,batch,1), tf.reshape(k, batch,1)])#batch*3
