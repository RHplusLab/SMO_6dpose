...
def check_6d_pose_2d_reprojection_s(model_3d_points, model_3d_diameter, rotation_gt, translation_gt, rotation_pred, translation_pred, camera_matrix, diameter_threshold = 0.1, pixel_threshold = 5.0, max_points = 1000):
    """ Check if the predicted 6D pose of a single example is considered to be correct using the 2D reprojection metric
...
def evaluate(
    ...
    for label in range(generator.num_classes()):
        ...
        true_positives_2d_projection_s  = np.zeros((0,))
        ...
        for i in tqdm(range(generator.size())):
            ...
            for d, d_rotation, d_translation in zip(detections, detections_rotations, detections_translations):
                ...
                if max_overlap >= iou_threshold and assigned_annotation not in detected_annotations:
                    ...
                    is_correct_2d_projection_s = check_6d_pose_2d_reprojection_s(model_3d_points,
                                                                             model_3d_diameter,
                                                                             rotation_gt = generator.axis_angle_to_rotation_mat(assigned_rotation),
                                                                             translation_gt = np.squeeze(assigned_translation),
                                                                             rotation_pred = generator.axis_angle_to_rotation_mat(d_rotation),
                    ...
                    if is_correct_2d_projection_s:
                        true_positives_2d_projection_s = np.append(true_positives_2d_projection_s, 1)
        ...
        #compute 2d projection - s accuracy
        accuracy_2d_projection_s = np.sum(true_positives_2d_projection_s) / num_annotations
        metric_2d_projection_s[label] = accuracy_2d_projection_s, num_annotations
